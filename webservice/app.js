...
class AppBootHook {
  ...
  async willReady() {
    ...
    const adminCount = await this.app.model.User.countDocuments({ isAdmin: true });
    if (adminCount === 0) {
      // 当前数据库中管理员的数量为0
      // 执行初始化管理员操作
      ...
      const initAdmin = new this.app.model.User({
        name: 'admin',
        passwordHash,
  ...
  async didReady() {
    // 应用已经启动完毕
    const transporter = nodemailer.createTransport(this.app.config.mail);
