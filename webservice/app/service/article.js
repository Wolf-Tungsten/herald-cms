...
class ArticleService extends Service {
  async prepare(article) {
    ...
    // 视频文件列表
    const videoList = await this.ctx.model.Resource.find({ articleId: article._id, type: 'video' });
    ...
    // 附件文件列表
    const appendFileList = await this.ctx.model.Resource.find({ articleId: article._id, type: 'appendFile' });
  ...
  async delete(articleId) {
    ...
    // 获取关联文件列表并删除
    const fileList = await this.ctx.model.Resource.find({ articleId });
    fileList.forEach(f => {
