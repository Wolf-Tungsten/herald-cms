...
class ArticleController extends Controller {
  ...
  async create() {
    ...
    const { columnId } = ctx.request.body;
    if (!columnId) {
      throw '未指定栏目';
    }
    const checkPermission = await this.service.permission.checkPermission(columnId);
    if (checkPermission === 'none') {
      throw '您无权在此栏目创建文章';
    }
    const code = this.ctx.helper.uuid().split('-')[0].toUpperCase();
    const newArticle = new this.ctx.model.Article({
      columnId,
  ...
  async save() {
    ...
    // 检查是否有冗余的图片文件
    const imageList = await this.ctx.model.Resource.find({ articleId, type: 'image' });
    imageList.forEach(image => {
      const isInUse = (content.indexOf(image.resourceName) !== -1) || article.coverImg === image.resourceName;
      if (!isInUse) {
        // 如果图片没有被引用，就要删除
  ...
  async publishOrReviewing() {
    ...
    const permission = await this.ctx.service.permission.checkArticlePermission(articleId);
    if (permission === 'none') {
      throw '无权操作';
    }
    const article = await this.ctx.model.Article.findById(articleId);
  ...
  async findArticleOfColumn() {
    let { columnId, pagesize = 10, page = 1, title } = await this.ctx.request.query;
    const permission = await this.ctx.service.permission.checkPermission(columnId);
    if (permission === 'publish') {
      page = +page;
      pagesize = +pagesize;
      const articleList = (await this.ctx.model.Article.find((title ? { columnId, title: { $regex: `.*(${title}).*` } } : { columnId }),
      ...
      const articleCount = await this.ctx.model.Article.countDocuments({ columnId });
      return { articleList, articleAmount: articleCount };
    } else if (permission === 'edit') {
      page = +page;
      pagesize = +pagesize;
      const articleList = (await this.ctx.model.Article.find((title ? { columnId, title: { $regex: `.*(${title}).*` }, authorId: (await this.ctx.getUserId()) } : { columnId, authorId: (await this.ctx.getUserId()) }),
        [ '_id', 'title', 'authorName', 'status', 'publishTime', 'code' ],
        {
          skip: pagesize * (page - 1),
          limit: pagesize,
  ...
  async findArticleOfOwn() {
    ...
    res.draft = await Promise.all(res.draft.map(async article => {
      const column = await this.ctx.model.Column.findOne({ _id: article.columnId }, [ 'name' ]);
  ...
  async findArticleNeedReviewing() {
    ...
    const { columnId } = await this.ctx.request.query;
    let reviewingList = [];
    if (columnId) {
      ...
      const columnList = await this.ctx.service.permission.getPermissionList();
      for (const column of columnList.publish) {
