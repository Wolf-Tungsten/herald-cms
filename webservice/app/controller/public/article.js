...
class PublicArticleController extends Controller {
  ...
  async getArticleList() {
    ...
    const rootColumn = await this.ctx.service.column.findColumnByCode(columnCode);
    if (!rootColumn) {
    ...
    const articleListRaw = await this.ctx.model.Article.find({ columnId: { $in: columnIdList },
    ...
    const articleAmount = await this.ctx.model.Article.countDocuments({ columnId: { $in: columnIdList },
      limited: { $in: [ false, showLimited ] },
      status: 'published',
      topFixed: false, // 检索的过程中忽略置顶文章
    });

    const topFixedListRaw = await this.ctx.model.Article.find({ columnId: { $in: columnIdList },
      limited: { $in: [ false, showLimited ] },
      status: 'published',
      topFixed: true,
    },
    ...
    for (const article of articleListRaw) {
      articleList.push(await this.ctx.service.article.prepare(article));
    }

    for (const article of topFixedListRaw) {
  ...
  async getArticleContent() {
    const { articleCode, token, identifier } = this.ctx.request.query;
    ...
    const articleAccess = await this.ctx.model.ArticleAccess.findOne({ token, valid: false, articleId: article._id, expireTime: { $gte: +this.ctx.helper.now() } });
    ...
    // 更新浏览量计数
    const articleView = new this.ctx.model.ArticleView({ articleId: article._id, timestamp: +this.ctx.helper.now(), identifier });
