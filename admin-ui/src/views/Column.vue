  ...
  <el-container style="height:100%;" v-loading="loading">
    ...
    <el-main style="margin-left:20px;">
      <div class="column-main">
        <div style="height:48px;text-align:left;line-height:48px;font-size:30px;">{{current.name}}</div>
        ...
        <div style="display:flex;flex-direction:column;align-items:flex-start;">
          <div style="margin:20px 0 15px 0;font-weight:bolder;">添加新栏目</div>
          <el-input style="width:360px;" v-model="newColumnName" placeholder="新栏目名称（以空格分割输入多个栏目名称）"></el-input>
          <el-button style="margin-top:15px;" type="primary" @click="createColumn">添加<i class="el-icon-plus el-icon--right"></i></el-button>
        </div>
  ...
  methods: {
    ...
    async createColumn(){
      ...
      for(let name of newColumnNames){
        let res = await this.$axios.post('/column/create',{name, parentId:this.currentOpId})
        if(!res.data.success){
